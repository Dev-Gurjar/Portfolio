  ...
  return (
    <div className="box-border caret-transparent gap-x-[normal] flex justify-start gap-y-[normal] pt-10 md:gap-x-10 md:gap-y-10 md:pt-40">
      <div className="sticky items-center self-start box-border caret-transparent flex flex-col max-w-xs w-auto z-40 top-40 md:flex-row md:max-w-sm md:w-full">
        <div className="absolute items-center bg-black box-border caret-transparent flex h-10 justify-center w-10 rounded-full left-3">
          <div className="bg-neutral-800 box-border caret-transparent h-4 w-4 border border-neutral-700 p-2 rounded-full border-solid"></div>
        </div>
        <h3 className="text-neutral-500 text-xl font-bold box-border caret-transparent hidden leading-7 min-h-0 min-w-0 pl-0 md:text-5xl md:block md:leading-[48px] md:min-h-[auto] md:min-w-[auto] md:pl-20">
          {props.year}
        </h3>
      </div>
      <div className="relative box-border caret-transparent w-full pl-20 pr-4 md:pl-4">
        <h3 className="text-neutral-500 text-2xl font-bold box-border caret-transparent block text-left mb-4 md:hidden">
          {props.year}
        </h3>
        <div className="box-border caret-transparent">
          <p className="text-neutral-200 text-xs box-border caret-transparent leading-4 mb-8 md:text-sm md:leading-5">
            {props.description}
          </p>
          {props.achievements && (
            <div className="box-border caret-transparent mb-8">
              {props.achievements.map((achievement, index) => (
                <div
                  key={index}
                  className="text-neutral-300 text-xs items-center box-border caret-transparent gap-x-2 flex leading-4 gap-y-2 md:text-sm md:leading-5"
                >
                  ✅ {achievement}
                </div>
              ))}
